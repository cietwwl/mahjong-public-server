...
import com.randioo.mahjong_public_server.comparator.CallCardListComparator;
import com.randioo.mahjong_public_server.comparator.CardComparator;
...
import com.randioo.mahjong_public_server.module.fight.component.dispatch.ClientDispatcher;
import com.randioo.mahjong_public_server.module.fight.component.dispatch.DebugDispatcher;
import com.randioo.mahjong_public_server.module.fight.component.dispatch.Dispatcher;
import com.randioo.mahjong_public_server.module.fight.component.dispatch.RandomDispatcher;
import com.randioo.mahjong_public_server.module.fight.component.fly.ZhamaChecker;
import com.randioo.mahjong_public_server.module.fight.component.fly.ZhamaResult;
import com.randioo.mahjong_public_server.module.fight.component.rule.SendBaiDaNotAllowHuRule;
...
public class FightServiceImpl extends ObserveBaseService implements FightService {
    ...
    public void readyGame(Role role) {
        ...
        if (game == null) {
            ...
            SessionUtils.sc(role.getRoleId(), SC.newBuilder().setFightReadyResponse(response).build());
            return;
        }
    ...
    public void queryGameConfig(Role role) {
        ...
        if (game == null) {
            SessionUtils.sc(
                    role.getRoleId(),
                    SC.newBuilder()
                            .setFightQueryGameConfigResponse(
                                    FightQueryGameConfigResponse.newBuilder().setErrorCode(
        ...
        SessionUtils.sc(
                ...
                SC.newBuilder()
                        .setFightQueryGameConfigResponse(
                                FightQueryGameConfigResponse.newBuilder().setGameConfigData(gameConfigData)
                                        .setCurrentRoundNumber(currentRoundNumber)).build());
    ...
    public void gameStart(Game game) {
        ...
        int baida = GameCache.getBaiDaCardNumSet().size() > 0 ? GameCache.getBaiDaCardNumSet().iterator().next() : 0;
        ...
        // 默认牌为0
        for (int j = 0; j < FightConstant.EVERY_INIT_CARD_COUNT; j++) {
            hideCards.add(0);
        }
        // 发送给每个玩家
        for (int i = 0; i < gameConfigData.getMaxCount(); i++) {
            ...
            // 隐藏别人的卡组
            this.hideOtherGameRoleInfoCards(roleGameInfo.cards, hideCards, i, gameConfigData.getMaxCount(), builder);
     ...
     */
    private void hideOtherGameRoleInfoCards(List<Integer> selfCards, List<Integer> hideCards, int selfSeat,
            int maxCount, SCFightStart.Builder scFightStartBuilder) {
        for (int i = 0; i < maxCount; i++) {
            // 如果是自己则要显示卡牌的内容
            List<Integer> cards = selfSeat == i ? selfCards : hideCards;
            FightStartRoleData fightStartRoleData = FightStartRoleData.newBuilder().addAllPai(cards).setSeat(i).build();
    ...
    public void dispatchCard(Game game) {
        ...
        cardComparator.getBaidaCardSet().addAll(GameCache.getBaiDaCardNumSet());
    ...
    private void calcMingGangScore(Game game, RoleGameInfo roleGameInfo, RoleGameInfo targetRoleGameInfo) {
        ...
        roleGameInfo.roundOverResult.mingGangCount++;
    ...
    private void calcAddGangScore(Game game, RoleGameInfo roleGameInfo) {
        ...
        for (RoleGameInfo info : game.getRoleIdMap().values()) {
            if (info.gameRoleId.equals(roleGameInfo.gameRoleId)) {
                ...
                info.roundOverResult.addGangCount++;
            } else {
    ...
    private void calcDarkGangScore(Game game, RoleGameInfo roleGameInfo) {
        ...
        for (RoleGameInfo info : game.getRoleIdMap().values()) {
            if (info.gameRoleId.equals(roleGameInfo.gameRoleId)) {
                ...
                info.roundOverResult.darkGangScore += darkAddScore;
                info.roundOverResult.darkGangCount++;
            } else {
                info.roundOverResult.score += -gangScore;
            }
        }
    ...
    private boolean checkQiangGang(Game game, int seat, RoleGameInfo roleGameInfo, Gang gang) {
        ...
        game.getCallCardLists().clear();
        game.getHuCallCardLists().clear();
        // boolean superQiangGang = true;
        // boolean zhuaHu = false;
        // boolean baiDaZhuaHu = false;
        // boolean baiDaQiangGang = false;
        //
        // if (!hasBaiDa(roleGameInfo, 0)) {
        // if (baiDaZhuaHu) {
        //
        // } else if (superQiangGang) {
        //
        // } else if (zhuaHu) {
        //
        // } else {
    ...
    private void over(Game game, int seat) {
        ...
        if (game.getFinishRoundCount() == 1) {
            // 大于一局就扣除燃点币
            GameRoundConfig config = GameRoundConfigCache
                    .getGameRoundByRoundCount(game.getGameConfig().getRoundCount());
    ...
    private void roundOver(Game game, boolean checkHu) {
        ...
        for (int i = 0; i < game.getRoleIdList().size(); i++) {
            ...
            int darkGangDeltaCount = gameRoundOverResult.darkGangCount - gameOverResult.darkGangCount;
            ...
            // 设置回合分数
            roleRoundOverInfoBuilder.setRoundScore(deltaScore).setAnGangCount(darkGangDeltaCount)
                    .setAnGangScore(darkGangDeltaScore).setMingGangCount(addGangDeltaCount + mingGangDeltaCount)
            ...
            gameOverResult.darkGangCount = gameRoundOverResult.darkGangCount;
    ...
    private void gameRoleIdSendCard(int card, Game game, String gameRoleId, boolean isSendTouchCard) {
        ...
        // ===========================================================
        GameConfigData gameConfigData = game.getGameConfig();
        boolean zhuaHu = gameConfigData.getZhuaHu() ? gameConfigData.getBaidaZhuaHu() ? true : false : false;
        List<Class<? extends CardList>> checkList = zhuaHu ? GameCache.getCheckCardListSequence() : GameCache
                .getCheckCardListOnlyMoHuSequence();
        for (int index = 0; index < game.getRoleIdList().size(); index++) {
            if (index == game.getCurrentRoleIdIndex()) {
                continue;
     ...
     */
    private boolean hasBaiDa(RoleGameInfo roleGameInfo, int card) {

        if (GameCache.getBaiDaCardNumSet().contains(card)) {
            return true;
        }
        for (int baida : GameCache.getBaiDaCardNumSet()) {
            int num = Collections.frequency(roleGameInfo.cards, baida);
            if (num > 0) {
                return true;
            }
        }
    ...
    private void newCardAdd2Cards(RoleGameInfo roleGameInfo) {
        ...
        comparator.getBaidaCardSet().addAll(GameCache.getBaiDaCardNumSet());
